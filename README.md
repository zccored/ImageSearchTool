...
- 引擎缓存策略：同一（索引前缀 + 关键参数）只加载一次，因此**重复搜图不再重新
  加载索引**（444k 瓦片库实测冷启动 9.4s → 热检索 0.8s）；建库前会自动释放缓存
  腾内存，索引被改写或参数变化时缓存自动失效。

### 索引存储格式：npz 与“侧车 .npy”

- 旧格式 `<前缀>.coarse.npz / <前缀>.fine.npz`：npz 成员**无法 mmap**，
  打开时必须整体解压进内存（444k 瓦片库实测 3.1s / 常驻 +1.18GB）。
- 新格式（侧车）：大数组拆成可 mmap 的 `.npy`
  （`paths/md5s/hu/fp/boxes/hu_stats/fine/fine_paths`），meta 里标记
  `"storage": "sidecar"`。**444k 瓦片库实测：打开 664ms（4.7×）、常驻内存 +76MB
  （省 1.1GB），检索结果完全一致**；读取异常或行数不符时自动回退旧 npz。
- 就地转换已有索引：
...
- **结果带命中框**：命中项返回原图像素坐标框（缩略图叠加红框），详情栏显示
  `命中框 (x0,y0)-(x1,y1)`；命中类型标注 `局部 / 整图 / 双`。
- **性能**（444,235 块瓦片库、RTX 4060 Laptop）：LSH 首查建表 ~2s，之后单次局部检索
  ~5s（查询切块 0.3s + 分块候选聚合 1.8s + 收敛精排 0.3s）；靶子用例
  （整图横切 ~24% 高度）Top1 命中原图、余弦 0.9987、命中框 (1045,384)-(1557,896)。
...
├── devtools/                        # 开发期回归脚本（verify_store / verify_gui_perf / check_real_index）
├── requirements.txt
└── hybrid_search/
...
- **局部（瓦片）索引 + 混合检索**：512px 切块、LSH 候选、查询侧切块聚合、
  命中框回传；GUI 三种检索模式。444k 瓦片库局部检索约 5s，靶子用例余弦 0.9987。
- **侧车 `.npy` 快载格式**：npz 成员无法 mmap（444k 瓦片库打开 3.1s / 常驻 +1.18GB），
  改为并列 `.npy` 后 **664ms / +76MB**；`compact` 命令与 GUI「优化索引存储」可就地转换，
  读取异常自动回退 npz。
...
- **重复图查验**：MD5 完全重复 + 指纹汉明近似重复（条带阻塞，O(N²)→近线性）；
  ...
  索引同步剔除。实测 F:\视频 42,414 张：89s 出结果，6,899 组 / 23,906 张 / 可释放 72.9GB。
- **大图对比预览**：双击行打开左右双图对比窗，独立缩放、拖动投放、小图栏复选框联动、
  F11 全屏、Esc 退出；分层采样 + 交互快速/停手精修，24MP 图稳态缩放 22ms/次。
- **缩略图磁盘缓存**：96px 小图 + 后台线程解码 + 滑出释放；2.4 万行窗口构建 0.5s、
  冷启动首屏 0.7s、二次打开 <0.1s、浏览内存 ~110MB。
- **修复**：结果网格缩略图失败导致点击错位/不足 Top-K；全局 LSH 缓存导致搜图后
  内存不释放；每次搜图重复加载索引（热检索 0.5s）；`add_tiles` 去重集合 O(N×M)
  （实测 530ms/张）；Windows 下被 mmap 的 `.npy` 无法替换。
