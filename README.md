...
> 真实图库 `F:\视频`：**37,683 张原图 + 444,189 块瓦片**，索引为侧车 `.npy` 格式。

| 场景 | 实测结果 |
| :--- | :--- |
| 整图索引加载（37,683 张） | **0.09 s** |
| 整图检索（库 37,683 张，热态） | **125 ms / 次**（中位数；首查含模型加载 2.8 s） |
| 瓦片索引加载（444,189 块） | **0.71 s** |
| 局部（瓦片）检索（444k 块） | **2.7 s 首查**（含 LSH 建表 ~1.7 s）→ 后续 **~1 s / 次** |
...
| 融合建库吞吐（真实 24MP 图，GPU） | **348 张/秒**（1,988 张 5.7 s，单遍解码出粗筛+ResNet） |
...
- **性能**（444,189 块瓦片库、RTX 4060 Laptop）：索引加载 **0.71 s**；首次查询含
  LSH 建表（~1.7 s）合计 **2.7 s**（查询切块 0.08 s + 分块候选聚合 2.20 s + 收敛精排
  0.30 s），之后 **~1 s / 次**；靶子用例（整图横切 ~24% 高度）Top1 命中原图、
  余弦 **0.9987**、命中框 (1045,384)-(1557,896)。
...
- **PNG 解码怎么选？** 默认 `--png-decoder cv2`（OpenCV 捆绑 libpng，比 Pillow 快
  ~1.3×，与 Pillow 逐像素一致：实测指纹汉明差 0%、ResNet 余弦 1.0000）；代价是
  个别坏 iCCP 文件的零星 stderr 警告，想彻底安静可切 `--png-decoder pillow`
  （慢一些）。JPEG/WebP/TIFF/BMP 走 OpenCV 快速路径。
